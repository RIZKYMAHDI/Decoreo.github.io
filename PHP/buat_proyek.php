...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Ambil nilai dari form dan beri nilai default jika tidak ada
    $building_type = $_POST['buildingType'] ?? '';
    $style = $_POST['style'] ?? '';
    $budget = $_POST['budget'] ?? 0; 
    $time = $_POST['time'] ?? 0;    
    $designer = $_POST['designer'] ?? '';

    // Pastikan semua field memiliki nilai sebelum menyimpan ke database
    if ($building_type && $style && $budget && $time && $designer) {
        // Koneksi ke database
        ...
        // Query untuk memasukkan proyek baru
        $sql = "INSERT INTO projects (user_id, building_type, design_style, budget, estimated_time, designer_choice) 
                VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("issdis", $user_id, $building_type, $style, $budget, $time, $designer);

        if ($stmt->execute()) {
            header("Location: proyek.php"); // Redirect ke halaman proyek setelah submit berhasil
            exit();
        } else {
        ...
        echo "Error: Semua field harus diisi!"; // Pesan jika ada field yang kosong
    }
}
?>
